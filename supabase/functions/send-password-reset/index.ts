...
serve(async (req) => {
  ...
  try {
    ...
    const redirectUrl = `${req.headers.get("origin") || "https://premium-channel-hub.lovable.app"}/reset-password`;
    ...
    const userName = user.user_metadata?.full_name || email.split("@")[0] || "Operador";
