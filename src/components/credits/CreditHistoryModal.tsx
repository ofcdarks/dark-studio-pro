...
export function CreditHistoryModal({ open, onOpenChange }: CreditHistoryModalProps) {
  const { user } = useAuth();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden bg-card border-primary/50 rounded-xl shadow-xl">
        <DialogHeader>
        ...
        <div className="space-y-4 overflow-y-auto max-h-[calc(85vh-100px)]">
          ...
          {loading ? (
            ...
            <>
              <TooltipProvider>
                <ScrollArea className={cn(showAll ? "h-[300px]" : "max-h-[250px]")}>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2 mb-1">
                                  <span className="font-medium text-sm text-foreground">
                                    {item.toolInfo.name}
                                    {formattedModel && <span className="text-muted-foreground font-normal text-xs"> – {formattedModel}</span>}
                                  </span>
                                  <span className={cn(
                                    "font-bold text-sm",
